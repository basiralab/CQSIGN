...
flags.DEFINE_string('dataset', 'PPI_EC', 'Dataset string.')
...
flags.DEFINE_integer('num_clusters', 20, 'Number of clusters.')
...
def evaluate_sens_spec(sess, model, val_features_batches, val_support_batches,
             y_val_batches, val_mask_batches, val_data, placeholders):
    """evaluate GCN model."""
    total_pred = []
    total_lab = []
    ...
    return utils.calc_sensitivity(total_pred, total_lab, FLAGS.multilabel), \
        utils.calc_specificity(total_pred, total_lab, FLAGS.multilabel)
...
def main(unused_argv):
    ...
    max_val_test_acc = 0.0
    max_val_test_f1 = 0.0
    max_val_test_sens = 0.0
    max_val_test_spec = 0.0
    ...
    for epoch in range(FLAGS.epochs):
        ...
        if FLAGS.validation:
            ...
            sens, spec = evaluate_sens_spec(sess, model, val_features_batches,
                                        val_support_batches, y_val_batches,
                                        val_mask_batches, val_data,
                                        placeholders)
            
            print(f"Val accuracy: {acc}, Val micro_f1: {micro}, Val Sens: {sens}, Val Spec: {spec}")
            ...
            if (acc > max_val_acc):
                ...
                sens, spec = evaluate_sens_spec(
                    sess, model, test_features_batches, test_support_batches,
                    y_test_batches, test_mask_batches, test_data, placeholders)
                
                max_val_test_acc = test_acc
                max_val_test_f1 = micro
