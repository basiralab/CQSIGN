...
def calc_specificity(y_pred, y_true, multilabel):
    if multilabel:
      y_pred[y_pred > 0] = 1
      y_pred[y_pred <= 0] = 0
    else:
...
def calc_sensitivity(y_pred, y_true, multilabel):
    if multilabel:
      y_pred[y_pred > 0] = 1
      y_pred[y_pred <= 0] = 0
    else:
...
def load_graphsage_data(dataset_path, dataset_str, normalize=True):
  ...
  ################################################
  print(f"Nodes for Validation: {val_data.shape}")
  print(f"Nodes for Test: {test_data.shape}")
  print(f"Nodes for Train: {train_data.shape}")
  ################################################
